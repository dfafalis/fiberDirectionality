...
The nonlinear system is formed by USER-DEFINED functions for the von Mises
    distributions on the semi-circle.
...
for mu, kap, pii in zip(locs_pr, kapp_pr, pi_pr):
    X_ud = pii*_vmf_pdf( x_, kap, mu, ll )
    X_tot += X_ud
    ax.plot(x_, X_ud, linewidth=2, \
            label=r'$\mu$ = {}, $\kappa$= {}, p= {} '.format(round(mu,3), round(kap,3), round(pii,3)))
#    X_st = pii*stats.vonmises.pdf( x_, kap, mu, scal_ )
#    ax.plot(x_, X_st, linestyle='--', linewidth=2, \
#            label=r'$\mu$ = {}, $\kappa$= {}, p= {} '.format(round(mu,3), round(kap,3), round(pii,3)))
...
X_tot += X_un
ax.plot(x_, X_un, linewidth=2, label='uniform, p= {} '.format(round(pu_pr,3)))
ax.plot(x_, X_tot, color='red', linewidth=3, label='mixture')
