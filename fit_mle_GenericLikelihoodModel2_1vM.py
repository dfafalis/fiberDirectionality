...
This code makes use of the "GenericLikelihoodModel" subclass of the package 
    ...
    Its generalization to mixtures of von Mises and Uniform distributions is 
        presented for a mixture of two von Mises in the file:
            fit_mle_GenericLikelihood_2vM_sc.py
    
    This version is using first the function 'stats.vonmises.pdf()' and then 
        takes the np.log() 
...
def vonMis_pdf( x, kappa=kappa_, loc=loc_ ):
    return stats.vonmises.pdf( x, kappa, loc )
...
class SinglevonMises(GenericLikelihoodModel):
    ...
    def nloglikeobs(self, params):
        kappa_ = params[0]
        loc_ = params[1]
        ll = np.sum( np.log( vonMis_pdf( self.endog, kappa=kappa_, loc=loc_ )))
